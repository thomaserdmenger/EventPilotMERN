...
export const postAddReviewCtrl = async (req, res) => {
  try {
    ...
    if (!reviewedUser)
      return res
        .status(400)
        .json({ errorMessage: "Could not review user. User does not exist." });

    if (userId === reviewedUserId)
      return res
        .status(400)
        .json({ errorMessage: "You could not review yourself" });
    ...
    if (text?.length > 140)
      return res
        ...
        .json({
          errorMessage:
            "The review text may contain a maximum of 140 characters.",
        });
    ...
    if (alreadyReviewed)
      return res
        .status(400)
        .json({ errorMessage: "You already reviewed this user." });
...
export const patchUpdateReviewCtrl = async (req, res) => {
  try {
    ...
    if (!reviewedUser)
      return res
        .status(400)
        .json("Could not review user. User does not exist.");

    if (userId === reviewedUserId)
      return res.status(400).json("You could not review yourself");

    if (text?.length > 140)
      return res
        .status(400)
        .json("The review text may contain a maximum of 140 characters.");
    ...
    res.json({
      ...
      reviews: {
        userId,
        stars: updatedReview.reviews.stars,
        text: updatedReview.reviews.text,
      },
    });
  } catch (error) {
    console.log(error);
...
export const deleteReviewCtrl = async (req, res) => {
  try {
    ...
    if (!review)
      return res
        .status(400)
        .json("Cannot delete review. Review does not exist.");
